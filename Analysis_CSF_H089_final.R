...
#total metabolites
metabolites <- as.data.frame(colnames(H089)[6:ncol(H089)] , check.names = F)
...
#save as csv
#write.csv(metabolites, file = "./data/metabolites_H089.csv", row.names = FALSE)
...
custom_order <- setdiff(seq(-12, 100, by = 1), 0)
...
top_anno <- HeatmapAnnotation(
  Temperature = anno_simple(
    H089$tb_mean,  # Use tb_mean as the annotation value
...
#H089n <- met.sc[, !grepl("^\\d+\\.\\d+/\\d+\\.\\d+$", colnames(met.sc))]
...
ccf_result <- ccf(H089n$`Dodecanedioic Acid`, 
                  H089n$tb_mean, 
                  lag.max = 23, 
                  plot = TRUE)
...
rownames(h.df) <-H089n %>% pull(1)#Making row names the samples_nr
...
custom_order <- setdiff(seq(1, 100, by = 1), 0)
custom_order <- as.character(custom_order) 
rownames(h.df) <- as.character(rownames(h.df))  
...
top_anno <- HeatmapAnnotation(
  Temperature = anno_simple(
    H089n$tb_mean,  # Use tb_mean as the annotation value
    col = temp_col_fun,  # Apply the color function
    border = TRUE  # Optional: Add border
  ),
  annotation_name_gp = gpar(fontsize = 10)  # Customize text size
...
#save as svg
#svg("./figures/H089_TorporCSF_heatmap_Tb_LAGG_by_phase_correlated_all_metabolites_Spearmanns.svg", width = 8, height = 12)
#draw(heat.ordered)
#dev.off()

################################################################################
#Psycadellic heatmap circle
################################################################################
library(circlize) 
...
# Ensure row names match metabolites in phase-sorted order
met_order_by_phase <- ccf_results_filtered %>%
  filter(Metabolite %in% colnames(h.df)) %>%
  arrange(Phase_deg, desc(abs(Max_Correlation))) %>%  # sort by phase, then strength
  pull(Metabolite)

mat <- mat[met_order_by_phase, ]  # Reorder rows

sample_order <- order(as.numeric(rownames(h.df)))
mat <- mat[, sample_order]

# Optional: ensure no duplicate rownames (for safety)
rownames(mat) <- make.unique(met_order_by_phase)

c <- rev(brewer.pal(5, "PRGn"))  # or your preferred palette
col_fun <- colorRamp2(c(-1, -1, 0, 1, 2), c)
...
circos.heatmap(
  ...
  col = col_fun,
  cluster = FALSE,
  split = NULL,  # Optional: can group by phase bins here
...
metabolites <- c(
  "D-Pantothenic Acid",  "Tiglylcarnitine", "4-Oxotretinoin",
  "Tyrosine", "Pyridoxal", "Phenylalanine", "Adenosine", "Androstenedione",
  "Dl-Carnitine", "Creatine", "Dopamine", "Choline", "Cytosine",
